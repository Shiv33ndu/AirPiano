...
#     hands = detector.find_hands(frame, draw=False)  # Delay drawing until after overlay
#     lm_lists = detector.find_positions(frame)

#     # Prepare overlay for transparency
#     overlay = frame.copy()
...
#         cv2.putText(overlay, note.upper(), (pos[0] - 20, pos[1] + 7), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
...
#     # Redraw hands over the blended frame
...
#             if len(lm_list) >= 9:
#                 index_tip = lm_list[8][1:]  # x, y of index finger
#                 index_pip = lm_list[6][1:]  # x, y of PIP joint
#                 thumb_tip = lm_list[4][1:]
#                 volume = 1.0

#             # Play only if finger is pressing downward
#             is_pressed = index_tip[1] > index_pip[1]

#             if is_pressed:
#                 dist = math.hypot(index_tip[0] - thumb_tip[0], index_tip[1] - thumb_tip[1])
#                 volume = max(0.2, min(1.0, (150 - dist) / 100))
...
#     # Update set of currently playing notes
#     new_playing = set()
#     if lm_lists:
#         for lm_list in lm_lists:
#             if len(lm_list) >= 9:
#                 index_tip = lm_list[8][1:]
#                 index_pip = lm_list[6][1:]
        ...
        for label, lmks in lm_lists:
            if len(lmks) >= 21:
                ...
                if label == "Right":  # thumb tip's x > mid means right hand (basic assumption)
                    ...
                    volume = max(0.2, min(1.0, palm_x / WIDTH))

                # Display volume bar always
                vol_height = int(volume * 300)
                cv2.rectangle(frame, (50, HEIGHT - 350), (90, HEIGHT - 50), (50, 50, 50), 2)
                cv2.rectangle(frame, (52, HEIGHT - 50 - vol_height), (88, HEIGHT - 50), (0, 255, 0), -1)
                cv2.putText(frame, f"{int(volume * 100)}%", (50, HEIGHT - 360), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
